...
class DefaultField extends PluginBase implements GeocoderFieldPluginInterface, ContainerFactoryPluginInterface {
  ...
  public function validateSettingsForm(array $form, FormStateInterface &$form_state) {
    ...
    // If the File Geocoder provider is the only selected.
    if (count($form_values['plugins']) == 1 && $form_values['plugins'][0] == 'file'
    ) {

      // Set error in case of Reverse Geocode operation (that is incompatible)
      if ($form_values['method'] == 'destination') {
        $form_state->setError($form['third_party_settings']['geocoder_field']['plugins']['file'], t('The File Geocoder provider is not compatible with the Reverse Geocode action.'));
      }
